...
from src.modules.creativity_engine import (
    Concept, CreativityType, NoveltyLevel, CreativeIdea, ConceptSpace,
    ConceptCombiner, NoveltyEvaluator, UtilityFilter, CreativityGenerator,
    CreativityAssessmentSystem, ConceptCombinationCreativityEngine
)
...
class TestConceptSpace:
    ...
    def test_compute_semantic_distance_same_category(self):
        ...
        space.add_concept(Concept(name="truck", category="transportation", attributes={"wheels": 6}))
    ...
    def test_compute_association_strength_with_relations(self):
        ...
        space.add_concept(Concept(
            name="engine",
            category="mechanical",
            relations=[("part_of", "car")]
        ))
        space.add_concept(Concept(
            name="car",
            category="transportation",
            relations=[("has_part", "engine")]
        ))
...
class TestConceptCombiner:
    ...
    def combiner_with_space(self):
        """创建带有概念空间的组合器"""
        space = ConceptSpace()
        space.add_concept(Concept(name="drone", category="technology", 
                                  attributes={"flight": True, "autonomous": True}))
        space.add_concept(Concept(name="delivery", category="logistics",
    ...
    def test_emergence_properties_generation(self, combiner_with_space):
        ...
        assert len(idea.emergence_properties) >= 2
        assert "bio-inspired design" in idea.emergence_properties or \
               "sustainable innovation" in idea.emergence_properties
...
class TestNoveltyEvaluator:
    ...
    def test_combination_rarity_calculation(self, evaluator_with_space):
        ...
        # 添加强关联概念
        space.add_concept(Concept(name="wheel", category="mechanical",
                                  relations=[("part_of", "car")]))
        space.add_concept(Concept(name="car", category="transportation",
                                  relations=[("has_part", "wheel")]))
...
class TestUtilityFilter:
    ...
    def test_filter_by_utility_threshold(self):
        ...
        ideas = []
        for i in range(10):
            c = Concept(name=f"concept_{i}", category="test", 
                       attributes={"value": i})
            idea = CreativeIdea(
                id=f"idea_{i}",
                title=f"Idea {i}",
    ...
    def test_focused_generation(self, generator_with_space):
        ...
        for idea in ideas:
            ...
            assert "AI" in concept_names or any(
                space.compute_semantic_distance("AI", c) < 0.5 
...
class TestConceptCombinationCreativityEngine:
    ...
    def test_generate_and_assess_workflow(self):
        ...
        concepts = [
            Concept(name="machine_learning", category="technology", attributes={"type": "AI"}, activation_level=0.9),
            Concept(name="healthcare", category="healthcare", attributes={"domain": "medical"}, activation_level=0.8),
            Concept(name="finance", category="business", attributes={"domain": "money"}, activation_level=0.8),
            Concept(name="education", category="social", attributes={"domain": "learning"}, activation_level=0.8),
            Concept(name="robotics", category="technology", attributes={"type": "automation"}, activation_level=0.8),
            Concept(name="art", category="culture", attributes={"type": "creative"}, activation_level=0.7),
        ]
        engine.initialize_concept_space(concepts)
    ...
    def test_full_creativity_pipeline(self):
        ...
        # 1. 初始化丰富的概念空间
        concepts = [
            Concept(name="neural_network", category="technology", 
                   attributes={"learning": "deep", "type": "AI"},
                   activation_level=0.9),
            Concept(name="creative_writing", category="art",
                   attributes={"medium": "text", "form": "narrative"},
                   activation_level=0.7),
            Concept(name="music_composition", category="art",
                   attributes={"medium": "audio", "form": "melody"},
                   activation_level=0.7),
            Concept(name="drug_discovery", category="healthcare",
                   attributes={"process": "research", "goal": "healing"},
                   activation_level=0.8),
            Concept(name="climate_modeling", category="science",
        ...
        divergent_ideas = engine.generate_creative_ideas(
            seed_concepts=["neural_network"],
            num_ideas=5,
            strategy="divergent"
        )
        
        focused_ideas = engine.generate_creative_ideas(
            seed_concepts=["neural_network", "creative_writing"],
            num_ideas=3,
            strategy="focused"
        )
        ...
        assessment = engine.assess_creativity(
            all_ideas,
            context="Neural network applications across domains"
        )
...
class TestCreativityTypes:
    ...
    def test_novelty_levels_ordering(self):
        """测试新颖性等级顺序"""
        levels = [NoveltyLevel.ORDINARY, NoveltyLevel.UNCOMMON, 
                 NoveltyLevel.NOVEL, NoveltyLevel.INNOVATIVE, 
