...
from src.modules.social_cognition import (
    EmotionType, IntentionType, BeliefStrength,
    Emotion, Belief, Desire, Intention, MentalState,
    TheoryOfMindEngine, EmpathyModule, IntentionRecognizer,
    DeceptionDetector, SocialLearningEngine, SocialNetworkModel,
...
class TestDeceptionDetector:
    ...
    def test_detect_deception_defensive(self):
        ...
        result = detector.detect_deception(
            statement='To be honest, I did not do it, believe me',
            context=''
        )
    ...
    def test_detect_deception_vague(self):
        detector = DeceptionDetector()
        result = detector.detect_deception(
            statement='Maybe perhaps possibly it could happen',
            context=''
        )
...
class TestSocialLearningEngine:
    ...
    def test_learn_social_norm(self):
        engine = SocialLearningEngine()
        sanctions = {
            'helping': 'reward and praise',
            'stealing': 'punishment and penalty'
        }
...
class TestComplexScenarios:
    ...
    def test_sally_anne_test(self):
        ...
        # 注册智能体
        sally = engine.register_agent('Sally', 
                                      ['Basket contains ball'],
                                      ['Play with ball'])
        anne = engine.register_agent('Anne',
                                     ['Box is empty'],
                                     [])
    ...
    def test_social_learning_chain(self):
        ...
        for i in range(3):
            engine.learn_from_interaction(
                'Student', 'Teacher',
                'reading books',
                'success and knowledge gain',
                'library'
            )
        
        # 学生应该学到积极行为
        student_state = engine.tom_engine.agent_models['Student']
        assert any('reading' in b.lower() or 'positive' in b.lower() 
                   for b in student_state.beliefs.keys())
    ...
    def test_belief_propagation(self):
        ...
        center = 'Influencer'
        for i in range(5):
            network.establish_relationship(
                center, f'Follower{i}',
                'fan', strength=0.9, trust=0.8
            )
