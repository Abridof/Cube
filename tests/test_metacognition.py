...
class TestCognitiveMetrics:
    ...
    def test_custom_values(self):
        metrics = CognitiveMetrics(attention_level=0.8, working_memory_load=0.6, error_rate=0.1)
...
class TestCognitiveLoadCalculator:
    ...
    def test_total_load_weighted(self):
        calc = CognitiveLoadCalculator(
            intrinsic_load_weight=0.4, extraneous_load_weight=0.3, germane_load_weight=0.3
        )
...
class TestStateDetector:
    ...
    def test_detect_flow_state(self):
        ...
        metrics = create_sample_metrics(
            attention=0.85, load=0.65, confidence=0.75, engagement=0.8, error=0.1
        )
    ...
    def test_detect_focused_state(self):
        detector = StateDetector()
        metrics = create_sample_metrics(attention=0.7, load=0.5, error=0.2)
        state = detector.detect_state(metrics)
        assert state == CognitiveState.FOCUSED

    def test_detect_overloaded_state(self):
        detector = StateDetector()
        metrics = create_sample_metrics(load=0.9, confidence=0.3, error=0.4)
        state = detector.detect_state(metrics)
        assert state == CognitiveState.OVERLOADED

    def test_detect_confused_state(self):
        detector = StateDetector()
        metrics = create_sample_metrics(load=0.65, confidence=0.4, error=0.4)
        state = detector.detect_state(metrics)
        assert state == CognitiveState.CONFUSED

    def test_detect_fatigued_state(self):
        detector = StateDetector()
        metrics = create_sample_metrics(attention=0.3, effort=0.8, engagement=0.3)
        state = detector.detect_state(metrics)
    ...
    def test_detect_bored_state(self):
        detector = StateDetector()
        metrics = create_sample_metrics(
            load=0.2, engagement=0.3, attention=0.3  # 低注意力也是无聊的特征
        )
        state = detector.detect_state(metrics)
    ...
    def test_detect_anxious_state(self):
        detector = StateDetector()
        metrics = create_sample_metrics(
            effort=0.8, confidence=0.3, error=0.5, load=0.5  # 中等负荷，避免被识别为困惑
        )
        state = detector.detect_state(metrics)
...
class TestStrategySelector:
    ...
    def test_context_critical_task_override(self):
        selector = StrategySelector()
        # 关键任务时，疲劳不应该休息
        strategy = selector.select_strategy(CognitiveState.FATIGUED, {"task_criticality": True})
        assert strategy == RegulationStrategy.DECREASE_CHALLENGE

    def test_context_time_pressure_override(self):
        selector = StrategySelector()
        # 时间紧迫时，过载不分解任务
        strategy = selector.select_strategy(CognitiveState.OVERLOADED, {"time_pressure": True})
        assert strategy == RegulationStrategy.PROVIDE_HINT
...
class TestMetacognitiveMonitor:
    ...
    def test_monitor_basic(self):
        ...
        metrics = create_sample_metrics()
        context = {"element_interactivity": 0.5, "prior_knowledge": 0.6}
    ...
    def test_monitor_state_detection(self):
        ...
        # 流状态指标 - 需要更高的注意力和参与度
        metrics = create_sample_metrics(
            attention=0.9, load=0.65, engagement=0.85, confidence=0.75, error=0.1
        )
        context = {}
    ...
    def test_monitor_regulation_triggered(self):
        monitor = MetacognitiveMonitor()
        # 过载状态 - 需要更高的负荷和更低的信心
        metrics = create_sample_metrics(load=0.95, confidence=0.2, effort=0.9)  # 高努力也表明过载
        context = {}
    ...
    def test_flow_probability(self):
        monitor = MetacognitiveMonitor()
        metrics = create_sample_metrics(attention=0.85, load=0.65, engagement=0.8)
        context = {}
...
class TestMetaLearner:
    ...
    def test_generate_recommendations(self):
        ...
        # 生成一些过载状态 - 需要更明显的过载指标
        for _ in range(10):
            metrics = create_sample_metrics(load=0.95, confidence=0.2, effort=0.9, attention=0.4)
            monitor.monitor(metrics, {})
...
class TestIntegration:
    ...
    def test_state_transitions(self):
        ...
        scenarios = [
            ...
            (
                create_sample_metrics(attention=0.4, load=0.95, confidence=0.2, effort=0.9),
                "OVERLOADED",
            ),
        ]
