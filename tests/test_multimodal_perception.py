...
class TestModalFeature(unittest.TestCase):
    ...
    def test_create_feature(self):
        """创建特征"""
        feat = ModalFeature(
            feature_type=FeatureType.SPATIAL, name="test_feature", value=42.0, confidence=0.95
        )
...
class TestPerceivedObject(unittest.TestCase):
    ...
    def test_create_object(self):
        """创建感知对象"""
        feat = ModalFeature(
            feature_type=FeatureType.SEMANTIC, name="category", value="cat", confidence=0.9
        )
        obj = PerceivedObject(
            object_id="obj_001",
...
class TestImagePerceiver(unittest.TestCase):
    ...
    def test_perceive_dict_with_dimensions(self):
        """感知结构化图像数据（带尺寸）"""
        image_dict = {"width": 800, "height": 600}
...
class TestMultimodalFusionEngine(unittest.TestCase):
    ...
    def test_fuse_single_modality(self):
        """融合单一模态"""
        feat = ModalFeature(
            feature_type=FeatureType.SEMANTIC, name="test", value=1.0, confidence=0.9
        )
        obj = PerceivedObject(
            object_id="test_001",
    ...
    def test_ucr_embedding_generation(self):
        """UCR 嵌入生成"""
        feat = ModalFeature(
            feature_type=FeatureType.STATISTICAL, name="count", value=50, confidence=0.9
        )
        obj = PerceivedObject(
            object_id="test_001",
    ...
    def test_cross_modal_alignment(self):
        ...
        # 创建两个有共同特征的对象
        feat1 = ModalFeature(
            feature_type=FeatureType.STATISTICAL, name="count", value=100, confidence=0.9
        )
        feat2 = ModalFeature(
