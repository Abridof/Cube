...
from modules.lifelong_learning import (
    PlasticityRule, TransferType, TaskCategory,
    SynapticWeight, Episode, TaskRepresentation,
    EpisodicBuffer, SynapticPlasticity, KnowledgeDistiller,
    SkillTransfer, NeuralPruning, LifelongLearner,
    create_sample_task, simulate_learning_session
)
...
class TestEpisode:
    ...
    def test_to_dict(self):
        """测试转换为字典"""
        episode = Episode("ep_001", TaskCategory.REASONING, 
                         [0.1, 0.2], [0.8], 0.9,
...
class TestEpisodicBuffer:
    ...
    def test_sample_for_replay(self):
        ...
        for i in range(10):
            episode = Episode(f"ep_{i}", TaskCategory.MEMORY, 
                            [0.1 * i], [0.9], 0.5 + 0.1 * i)
            buffer.add_episode(episode)
...
class TestSkillTransfer:
    ...
    def test_calculate_transfer_potential_same_category(self):
        ...
        task1 = create_sample_task("task1", TaskCategory.PERCEPTION, 0.5, 
                                  ["vision", "attention"])
        task2 = create_sample_task("task2", TaskCategory.PERCEPTION, 0.6,
                                  ["vision", "memory"])
    ...
    def test_transfer_knowledge_positive(self):
        ...
        task1 = create_sample_task("task1", TaskCategory.REASONING, 0.5, 
                                  ["logic", "analysis"])
        task2 = create_sample_task("task2", TaskCategory.REASONING, 0.6,
    ...
    def test_find_best_source_task(self):
        ...
        target = create_sample_task("target", TaskCategory.REASONING, 0.6,
                                   ["logic", "deduction"])
...
class TestIntegration:
    ...
    def test_catastrophic_forgetting_prevention(self):
        ...
        # 记录任务 1 的突触权重
        weights_after_task1 = {
            k: v.weight for k, v in learner.synaptic_plasticity.synapses.items()
        }
