...
class TestLLMResponse(unittest.TestCase):
    ...
    def test_create_response(self):
        ...
        response = LLMResponse(
            text="Hello", tokens_used=10, model="gpt-3.5-turbo", latency_ms=100.5, success=True
        )
...
class TestLLMClientInit(unittest.TestCase):
    ...
    def test_custom_init(self):
        """自定义初始化"""
        client = LLMClient(
            api_key="test-key", model="gpt-4", max_tokens=2000, temperature=0.5, timeout=60
        )
...
class TestRetryMechanism(unittest.TestCase):
    ...
    def test_retry_config(self):
        """测试重试配置"""
        client = LLMClient(retry_attempts=5, retry_delay=0.5)
