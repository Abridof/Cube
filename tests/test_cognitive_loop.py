...
# 导入被测模块
from src.modules.cognitive_loop import CognitiveLoopController, CognitiveEvent, LoopPhase
...
class TestCognitiveEvent(unittest.TestCase):
    ...
    def test_event_to_dict(self):
        """测试事件序列化"""
        event = CognitiveEvent(
            phase=LoopPhase.REASON, timestamp=1234567890.0, input_data="input", output_data="output"
        )
...
class TestCognitiveLoopController(unittest.TestCase):
    ...
    def setUp(self):
        """初始化测试环境"""
        self.config = {"learning_rate": 0.01, "exploration_bonus": 0.2, "reflection_threshold": 0.7}
    ...
    def test_reflect_phase_high_performance(self):
        ...
        cycle_data = {"feedback": {"reward": 1.0}, "decision": {"confidence": 0.9}}
    ...
    def test_reflect_phase_low_performance(self):
        ...
        cycle_data = {"feedback": {"reward": -0.2}, "decision": {"confidence": 0.3}}
    ...
    def test_full_cycle(self):
        """测试完整认知循环"""
        result = self.controller.run_cycle(input_data="test question", goals=["learn_something"])
    ...
    def test_autonomous_session(self):
        """测试自主会话"""
        # 运行一个非常短的会话（1 秒）
        results = self.controller.run_autonomous_session(duration_seconds=1, tick_rate=0.1)
