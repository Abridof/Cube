...
class TestThoughtNode:
    ...
    def test_create_node(self):
        """创建基本节点"""
        node = ThoughtNode(id="test_1", content="这是一个测试节点", depth=0)
    ...
    def test_uct_score_visited(self):
        """已访问节点的 UCT 分数应有限"""
        node = ThoughtNode(id="test", content="test", visits=10, total_reward=8.0)
...
class TestReasoningStep:
    ...
    def test_create_step(self):
        step = ReasoningStep(step_number=1, content="第一步分析", method="analyze", confidence=0.9)
...
class TestTreeOfThought:
    ...
    def test_mcts_search(self):
        ...
        trace, nodes = tot.mcts_search("游戏策略优化", num_simulations=50)
    ...
    def test_custom_evaluator(self):
        ...
        trace, nodes = tot.search("代码优化方案", evaluator=custom_eval)
...
class TestMultiStepPlanner:
    ...
    def test_replan_after_failure(self):
        ...
        new_plan = planner.replan(
            original_plan, failed_step_index=2, new_context={"new_info": "发现新情况"}
        )
...
class TestSlowThinkingEngine:
    ...
    def test_different_strategies(self):
        ...
        result_cot = engine.reason("优化问题", strategy=ReasoningStrategy.CHAIN_OF_THOUGHT)
        result_tot = engine.reason("优化问题", strategy=ReasoningStrategy.TREE_OF_THOUGHT)
    ...
    def test_validation_enabled(self):
        """测试验证功能"""
        engine = SlowThinkingEngine()
        result = engine.reason("逻辑推理题", enable_validation=True)
    ...
    def test_validation_disabled(self):
        """关闭验证"""
        engine = SlowThinkingEngine()
        result = engine.reason("简单问题", enable_validation=False)
    ...
    def test_plan_and_execute(self):
        """规划并执行"""
        engine = SlowThinkingEngine()
        result = engine.plan_and_execute("完成数据分析项目", plan_type="analysis")
    ...
    def test_compare_strategies(self):
        """比较不同策略"""
        engine = SlowThinkingEngine()
        strategies = [ReasoningStrategy.CHAIN_OF_THOUGHT, ReasoningStrategy.TREE_OF_THOUGHT]

        result = engine.compare_strategies("复杂优化问题", strategies)
...
class TestConvenienceFunctions:
    ...
    def test_deep_reason_with_strategy(self):
        """指定策略的深度推理"""
        result = deep_reason("优化算法", strategy="tree_of_thought")
...
class TestIntegration:
    ...
    def test_full_reasoning_pipeline(self):
        """完整推理流程"""
        engine = SlowThinkingEngine(
            config={"max_cot_steps": 15, "max_tot_depth": 8, "beam_width": 4}
        )
        ...
        # 思维树推理
        result2 = engine.reason("同样的逻辑问题", strategy=ReasoningStrategy.TREE_OF_THOUGHT)
    ...
    def test_complex_planning(self):
        ...
        result = engine.plan_and_execute(
            goal, plan_type="creative", context={"constraints": ["时间紧迫", "资源有限"]}
        )
