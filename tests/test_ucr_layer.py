...
class TestVectorEmbedding(unittest.TestCase):
    ...
    def test_creation_and_norm(self):
        """测试创建和范数计算"""
        embedding = VectorEmbedding(id="emb_1", vector={"word1": 0.5, "word2": 0.8})

        self.assertGreater(embedding.norm, 0)
        self.assertAlmostEqual(embedding.norm, (0.5**2 + 0.8**2) ** 0.5, places=6)
...
class TestCognitiveUnit(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        symbolic = SymbolicNode(
            id="sym_1", entity_type=EntityType.CONCEPT, label="Test", definition="Test definition"
        )
        vector = VectorEmbedding(id="vec_1", vector={"test": 1.0})

        unit = CognitiveUnit(id="unit_1", symbolic=symbolic, vector=vector, tags={"tag1", "tag2"})
    ...
    def test_serialization(self):
        """测试序列化/反序列化"""
        symbolic = SymbolicNode(
            id="sym_2", entity_type=EntityType.ACTION, label="Action", definition="An action"
        )
        vector = VectorEmbedding(id="vec_2", vector={"action": 1.0})

        unit = CognitiveUnit(id="unit_2", symbolic=symbolic, vector=vector, tags={"test"})
...
class TestUnifiedRepresentationEngine(unittest.TestCase):
    ...
    def test_search_by_similarity(self):
        ...
        for unit, score in results:
            ...
            if any(
                term in label or term in definition
                for term in ["machine", "learning", "algorithms", "concept"]
            ):
                found_relevant = True
                break
