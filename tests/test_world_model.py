...
class TestTransitionLearning(unittest.TestCase):
    ...
    def test_causal_graph_population(self):
        ...
        for i in range(5):
            self.wm.observe({
                'cause': (i, StateType.PHYSICAL),
                'effect': (i * 2, StateType.PHYSICAL)
            })
...
class TestPersistence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(5):
            self.wm.observe({
                'x': (i, StateType.PHYSICAL),
                'y': (i * 2, StateType.PHYSICAL)
            })
