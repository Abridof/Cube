...
class StructureAligner:
    ...
    def _build_relation_graph(self, relations: List[Tuple[str, str, str]]) -> Dict[str, Set[str]]:
        """构建关系图"""
    ...
    def _compute_relation_similarity(
        self, rel1: Tuple[str, str, str], rel2: Tuple[str, str, str]
    ) -> float:
        """计算两个关系的相似度"""
        # 关系类型相似度
...
class AnalogyGenerator:
    ...
    def generate_analogy(
        ...
        """
        # 结构对齐
        alignments = self.aligner.align_structures(source_relations, target_relations)

        if not alignments:
            return None

        # 生成映射假设
        mappings = self._generate_mappings(source_relations, target_relations, alignments)
        ...
        # 计算创造性分数
        creativity_score = self._compute_creativity(source_domain, target_domain, analogy_type)

        # 提取可迁移知识
        transferable = self._extract_transferable_knowledge(mappings, analogy_type)
    ...
    def _compute_creativity(
        self, source_domain: str, target_domain: str, analogy_type: AnalogyType
    ) -> float:
        """计算类比的创造性分数"""
        # 基于领域距离和类比类型的创造性评估
...
class KnowledgeTransferEngine:
        ...
        for mapping in analogy.mappings:
            if mapping.source_element in source_knowledge:
                strategy = self._select_transfer_strategy(
                    mapping, source_knowledge[mapping.source_element], target_context
                )

                transferred[mapping.target_element] = self._apply_transfer(
                    source_knowledge[mapping.source_element], mapping, strategy, target_context
                )
    ...
    def _apply_transfer(
        ...
        if strategy == "direct_mapping":
            ...
            # 适应性调整：根据目标上下文微调
            return self._adapt_knowledge(source_knowledge, mapping, target_context)
...
class AnalogyReasoningEngine:
    ...
    def reason_by_analogy(
        ...
        analogy = self.generator.generate_analogy(
            source_domain, target_domain, source_relations, target_relations, analogy_type
        )
    ...
    def _extract_insights(self, analogy: AnalogyMapping) -> List[str]:
        """提取洞察"""
        insights = []
        ...
        # 高置信度映射提供可靠洞见
        high_conf_mappings = [m for m in analogy.mappings if m.confidence > 0.8]
        if high_conf_mappings:
            insights.append(f"识别{len(high_conf_mappings)}个高置信度映射关系")

        # 结构对齐洞察
        if len(analogy.structural_alignments) > 2:
            insights.append("发现深层结构相似性")
            ...
            for j in range(i + 1, len(domains)):
                ...
                for analogy_type in [
                    ...
                    result = self.reason_by_analogy(
                        source_name, target_name, source_struct, target_struct, analogy_type
                    )

                    if result and result["analogy"]["creativity_score"] >= min_creativity:
                        # 从结果中重建 AnalogyMapping
                        analogy_data = result["analogy"]
                        creative_analogies.append(self._dict_to_analogy(analogy_data))

        # 按创造性分数排序
        creative_analogies.sort(key=lambda a: a.creativity_score, reverse=True)
...
def demonstrate_analogy():
    ...
    water_domain = {
        ...
        "knowledge": {"水泵": "提供动力源", "阀门": "调节流量", "水压": "压力差产生流动"},
    }
    ...
    result = engine.reason_by_analogy(
        "水力学系统", "电路系统", water_domain, circuit_domain, AnalogyType.STRUCTURAL
    )
    ...
    evolution_domain = {
        ...
        "knowledge": {"自然选择": "适者生存", "变异": "引入多样性", "繁殖": "优势基因传递"},
    }
    ...
    result = engine.reason_by_analogy(
        "生物进化", "遗传算法", evolution_domain, algorithm_domain, AnalogyType.PROCEDURAL
    )
