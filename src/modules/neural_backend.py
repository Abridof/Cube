...
class Tensor:
    ...
    def __add__(self, other: "Tensor") -> "Tensor":
        return Tensor(data=[a + b for a, b in zip(self.data, other.data)], shape=self.shape)
...
class NeuralUCREncoder:
    ...
    def _init_weights(self):
        ...
        self.W1 = [
            [random.gauss(0, scale1) for _ in range(self.input_dim)] for _ in range(self.hidden_dim)
        ]
    ...
    def forward(self, x: List[float]) -> List[float]:
        ...
        # Cache for backprop
        self._cache = {"input": x, "h1_pre": h1_pre, "h1": h1, "h2_pre": h2_pre, "output": h2}
    ...
    def backward(self, grad_output: List[float]):
        ...
        # Layer 2 gradients
        grad_W2 = [
            [grad_h2[i] * h1[j] for j in range(self.hidden_dim)] for i in range(self.output_dim)
        ]
        grad_b2 = grad_h2[:]

        # Backprop to h1
        grad_h1 = matrix_vector_mult(
            [[self.W2[i][j] for i in range(self.output_dim)] for j in range(self.hidden_dim)],
            grad_h2,
        )

        # ReLU gradient
        grad_h1_pre = [g * (1.0 if h > 0 else 0.0) for g, h in zip(grad_h1, h1_pre)]

        # Layer 1 gradients
        grad_W1 = [
            [grad_h1_pre[i] * x[j] for j in range(self.input_dim)] for i in range(self.hidden_dim)
        ]
        grad_b1 = grad_h1_pre[:]
...
class ContrastiveLearner:
    ...
    def _update_for_contrastive_loss(
        self, anchor, positive, negatives, pos_sim: float, neg_sims: List[float]
    ):
...
class IntrinsicMotivation:
    ...
    def compute_total_drive(
        self, state: str, prediction_error: float, concept: Optional[Dict] = None
    ) -> float:
        """
...
class NeuralSymbolicLearner:
    ...
    def ingest_and_learn(self, source: str, query: str, train_epochs: int = 5) -> Dict[str, Any]:
        ...
        for ucr in ucrs:
            if ucr.get("entity_type") == "RELATION":
                rule = {"head": ucr.get("head", {}), "tail": ucr.get("tail", {}), "relation": ucr}
    ...
    def _extract_training_pairs(self, ucrs: List[Dict]) -> List[Tuple[Dict, Dict, List[Dict]]]:
        ...
        for etype, items in by_type.items():
            ...
            for i, anchor in enumerate(items):
                ...
                negatives = [items[(i + j) % len(items)] for j in range(2, min(5, len(items)))]
    ...
    def explore_with_motivation(self, queries: List[str]) -> Dict[str, Any]:
        ...
        for query in queries:
            ...
            drive = self.motivation.compute_total_drive(state_repr, pred_error, {"symbol": query})

            # Decide whether to explore
            if drive > 0.3:
                result = self.ingest_and_learn("wikipedia", query, train_epochs=2)
                trajectory.append(
                    {"query": query, "drive": drive, "explored": True, "result": result}
                )
            else:
                trajectory.append({"query": query, "drive": drive, "explored": False})
