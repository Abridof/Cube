...
class ConceptCombiner:
    ...
    def combine_two_concepts(
        self, concept1_name: str, concept2_name: str, combination_type: str = "fusion"
    ...
    def _fusion_combine(self, c1: Concept, c2: Concept) -> CreativeIdea:
        ...
        description = (
            f"A creative fusion of {c1.name} and {c2.name}, combining their core features."
        )
    ...
    def _generate_emergence_properties(
        self, c1: Concept, c2: Concept, combo_type: str
    ) -> List[str]:
        """生成涌现属性"""
        emergence = []
    ...
    def _calculate_utility(self, c1: Concept, c2: Concept, combined_attrs: Dict) -> float:
        """计算实用性分数"""
        # 基于属性数量和多样性
    ...
    def _calculate_feasibility(self, c1: Concept, c2: Concept, combined_attrs: Dict) -> float:
        """计算可行性分数"""
        # 基于概念本身的可行性
...
class UtilityFilter:
    ...
    def filter_ideas(
        self, ideas: List[CreativeIdea], min_utility: float = 0.5, min_feasibility: float = 0.4
    ) -> List[CreativeIdea]:
        """筛选出具有实用价值的创意"""
        filtered = []
...
class CreativityGenerator:
    ...
    def _divergent_generation(
        self, seed_concepts: List[str], num_ideas: int, creativity_types: List[CreativityType]
    ) -> List[CreativeIdea]:
        """发散式生成：广泛探索概念空间"""
        ideas = []
        ...
        if not seed_concepts:
            ...
            if not available_concepts:
                return ideas
            seed_concepts = random.sample(available_concepts, min(5, len(available_concepts)))
    ...
    def _calculate_overall_score(self, idea: CreativeIdea) -> float:
        """计算综合创造力分数"""
        # 综合新颖性、实用性和可行性
        weights = {"novelty": 0.4, "utility": 0.35, "feasibility": 0.25}
...
class CreativityAssessmentSystem:
    ...
    def assess_creativity(self, ideas: List[CreativeIdea], context: str = "") -> Dict:
        """评估整体创造力水平"""
        if not ideas:
            return {
                "overall_score": 0.0,
    ...
    def _calculate_diversity(self, ideas: List[CreativeIdea]) -> float:
        ...
        # 基于概念组合的多样性
        concept_sets = [frozenset(c.name for c in idea.combined_concepts) for idea in ideas]
...
class ConceptCombinationCreativityEngine:
    ...
    def get_top_ideas(
        self, ideas: List[CreativeIdea], top_k: int = 5, criterion: str = "overall"
    ) -> List[CreativeIdea]:
        """获取顶级创意"""
    ...
    def export_idea_report(self, ideas: List[CreativeIdea], format: str = "text") -> str:
        """导出创意报告"""
        if format == "text":
