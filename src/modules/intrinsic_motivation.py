...
class PredictionErrorCalculator:
    ...
    def calculate_error(self, predicted: Any, actual: Any, 
                       var_name: str = "default",
                       context: Optional[Dict] = None) -> PredictionError:
        """
        计算预测误差
    ...
    def get_surprising_errors(self, threshold: float = 0.7, 
                             limit: int = 10) -> List[PredictionError]:
        """获取高惊讶度的误差"""
        surprising = [e for e in self.errors.values() 
                     if e.surprise_level >= threshold]
...
class InformationGainCalculator:
    ...
    def calculate_information_gain(self, 
                                   ...
                                   learning_event: str = "") -> InformationGain:
        """
        计算信息增益
        ...
        # 记录学习曲线
        timestamp = time.time()
        cumulative_gain = sum(g.information_gain for g in self.gains.values() 
    ...
    def get_knowledge_growth_rate(self, knowledge_area: str, 
...
class IntrinsicMotivationEngine:
    ...
    def process_prediction(self, predicted: Any, actual: Any,
                          var_name: str = "default",
                          context: Optional[Dict] = None) -> IntrinsicReward:
        """
        处理预测结果，生成内在奖励
        ...
        """
        # 计算预测误差
        error = self.error_calculator.calculate_error(
            predicted, actual, var_name, context
        )
        ...
        reward = IntrinsicReward(
            ...
            reward_type=MotivationType.SURPRISE if surprise_reward > competence_reward else MotivationType.COMPETENCE,
    ...
    def process_learning_event(self, 
                               ...
                               after_probs: List[float],
                               knowledge_area: str = "general",
                               learning_event: str = "") -> IntrinsicReward:
        """
        处理学习事件，生成内在奖励
        ...
        # 生成学习进步奖励
        progress_reward = gain.information_gain * self.motivation_weights[MotivationType.LEARNING_PROGRESS]
    ...
    def generate_exploration_goal(self, 
                                  ...
                                  motivation_type: MotivationType,
                                  expected_gain: float,
                                  target_state: Any = None) -> ExplorationGoal:
        """
        生成探索目标
...
def demo_intrinsic_motivation():
    ...
    for i in range(10):
        ...
        reward = engine.process_prediction(
            predicted=predicted,
            actual=actual,
            var_name="linear_function",
            context={'step': i}
        )
        
        print(f"Step {i}: predicted={predicted:.2f}, actual={actual:.2f}, "
