...
class KnowledgeGraph:
    ...
    ) -> List[Tuple[CognitiveUnit, KnowledgeEdge]]:
        """获取邻居节点"""
        neighbors = []
    ...
    def get_subgraph(self, center_id: str, max_depth: int = 2) -> Dict[str, Any]:
        ...
        subgraph_nodes = {
            nid: self.nodes[nid].to_dict() for nid in nodes_to_include if nid in self.nodes
        }
...
class MetaLearner:
    ...
    def select_strategy(
        self, problem_description: str, domain: str, context: Optional[Dict] = None
    ) -> LearningStrategy:
        """根据问题选择最优学习策略"""
        # 简单实现：基于领域匹配
        best_strategy = None
    ...
    def test_hypothesis(self, hypothesis_id: str, evidence: str, supports: bool) -> Hypothesis:
        """测试假设"""
        ...
        logger.info(
            f"Tested hypothesis {hypothesis_id}: {'supported' if supports else 'contradicted'}"
        )
        return hypothesis
            ...
            for id2 in node_ids[i + 1 :]:
                ...
                if unit1.vector and unit2.vector:
                    ...
                    if similarity >= threshold:
                        ...
                        logger.info(
                            f"Merged similar concepts: {id2} -> {id1} (similarity: {similarity:.3f})"
                        )
...
class EnhancedMemoryBank:
    ...
    def __init__(
        self,
        storage_path: str = "memory_bank.json",
    ...
    def add_knowledge(
        ...
        unit = self.ucr_engine.create_unit(
            content, content_type=content_type, domain=domain, tags=tags
        )
        ...
        similar_units = self.ucr_engine.search_by_similarity(
            unit.symbolic.definition, threshold=0.3, limit=5
        )
    ...
    def retrieve(
        ...
        # 领域过滤
        if domain_filter:
            results = [(unit, score) for unit, score in results if domain_filter in unit.tags]

        return results[:top_k]

    def learn_from_interaction(
        self, problem: str, solution: str, success: bool, domain: str = "general"
    ) -> Dict[str, Any]:
        """
        从交互中学习
        ...
        problem_unit = self.add_knowledge(
            problem, content_type="text", domain=domain, tags={"problem", domain}
        )

        solution_unit = self.add_knowledge(
            solution, content_type="text", domain=domain, tags={"solution", domain}
        )
    ...
    def get_knowledge_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "entity_types": {
                et.value: len(self.ucr_engine.index_by_type.get(et, set())) for et in EntityType
            },
            "strategy_stats": self.meta_learner.get_strategy_stats(),
            "active_hypotheses": len(
                [h for h in self.meta_learner.hypotheses.values() if h.status == "pending"]
            ),
        }
...
if __name__ == "__main__":
    ...
    hyp = memory.meta_learner.generate_hypothesis(
        "Using meaningful variable names improves code readability", [unit1.id]
    )
