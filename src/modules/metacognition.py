...
class CognitiveLoadCalculator:
    ...
    def calculate_germane_load(self,
                                schema_construction: float,
                                automation_level: float) -> float:
        """计算相关认知负荷（用于学习的负荷）"""
...
class StateDetector:
    ...
    def detect_state(self, metrics: CognitiveMetrics) -> CognitiveState:
        ...
        if (attention > 0.7 and 0.5 < load < 0.8 and 
            engagement > 0.7 and confidence > 0.6 and error_rate < 0.2):
    ...
    def update_history(self, state: CognitiveState):
        ...
        if len(self.state_history) >= 2:
            ...
            if total > 0:
                count = sum(1 for i, s in enumerate(self.state_history[:-1]) 
...
class StrategySelector:
    ...
    def select_strategy(self, 
                       ...
                       context: Dict[str, Any]) -> RegulationStrategy:
        """根据当前状态选择调节策略"""
...
class DifficultyAdjuster:
    ...
    def get_recommended_difficulty(self, 
                                    skill_level: float,
                                    challenge_seeking: float = 0.5) -> float:
        """根据技能水平和挑战寻求度推荐难度"""
        # 流理论：难度应略高于技能水平
...
class MetacognitiveMonitor:
    ...
    def monitor(self, metrics: CognitiveMetrics, 
        ...
        if strategy:
            ...
            self.strategy_selector.record_outcome(
                strategy, 
                1.0 if outcome and 'success' in outcome.lower() else 0.0
            )
    ...
    def _apply_regulation(self, strategy: RegulationStrategy, 
    ...
    def _calculate_flow_probability(self, metrics: CognitiveMetrics, 
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        success_rate = (
            ...
            if self.total_regulation_count > 0 else 0.0
        )
...
class MetaLearner:
    ...
    def analyze_patterns(self) -> List[Dict[str, Any]]:
        ...
        if morning_logs and afternoon_logs:
            morning_flow = sum(1 for l in morning_logs if l.state == CognitiveState.FLOW) / len(morning_logs)
            afternoon_flow = sum(1 for l in afternoon_logs if l.state == CognitiveState.FLOW) / len(afternoon_logs)
