...
class Emotion:
    ...
    def _default_valence(self) -> float:
        positive_emotions = {
            EmotionType.JOY, EmotionType.PRIDE, EmotionType.GRATITUDE,
            EmotionType.HOPE, EmotionType.RELIEF
        }
        negative_emotions = {
            EmotionType.SADNESS, EmotionType.ANGER, EmotionType.FEAR,
            EmotionType.DISGUST, EmotionType.GUILT, EmotionType.SHAME,
            EmotionType.ENVY, EmotionType.CONTEMPT, EmotionType.DISAPPOINTMENT,
            EmotionType.REGRET
        }
    ...
    def _default_arousal(self) -> float:
        high_arousal = {
            ...
            EmotionType.JOY, EmotionType.DISGUST
        }
        low_arousal = {
...
class Belief:
    ...
    def update_strength(self, new_evidence_strength: float, 
                       evidence_reliability: float = 0.8) -> None:
        """根据新证据更新信念强度"""
...
class MentalState:
    ...
    def add_belief(self, content: str, strength: BeliefStrength = BeliefStrength.POSSIBLE,
    ...
    def add_intention(self, intention_type: IntentionType, target: str,
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'dominant_emotion': self.get_dominant_emotion().to_dict() if self.get_dominant_emotion() else None,
...
class TheoryOfMindEngine:
    ...
    def infer_intention(self, agent_id: str, observed_actions: List[str],
                        context: Optional[str] = None) -> Optional[Intention]:
        """从观察到的行为推断意图"""
        if agent_id not in self.agent_models:
            raise ValueError(f"Unknown agent: {agent_id}")
    ...
    def infer_emotion(self, agent_id: str, situation: str,
                      agent_goals: List[str] = None) -> List[Emotion]:
        """基于情境推断他人的情绪"""
        if agent_id not in self.agent_models:
            raise ValueError(f"Unknown agent: {agent_id}")
        ...
        for emotion_type, keywords in situation_keywords.items():
            ...
            if match_count > 0:
                ...
                if agent_goals:
                    ...
                    if goal_achievement > 0.5 and emotion_type in [EmotionType.JOY, EmotionType.PRIDE, EmotionType.RELIEF]:
                        intensity = min(intensity + 0.2, 1.0)
                    elif goal_achievement < 0.3 and emotion_type in [EmotionType.SADNESS, EmotionType.ANGER, EmotionType.DISAPPOINTMENT]:
                        intensity = min(intensity + 0.2, 1.0)
    ...
    def second_order_inference(self, observer_id: str, target_id: str,
                               proposition: str) -> Belief:
        ...
        target_model_as_seen_by_observer = self._construct_perspective(
            observer_state, target_id
        )
    ...
    def false_belief_test(self, agent_id: str, actual_state: str,
                          agent_observation: str) -> Tuple[Belief, bool]:
        """
        ...
        # 基于 agent 的观察推断其信念
        inferred_belief = Belief(
            content=agent_observation,
            strength=BeliefStrength.LIKELY,
            source='observation'
        )
    ...
    def update_agent_model(self, agent_id: str, new_observation: str,
                           new_action: Optional[str] = None) -> None:
        """根据新的观察更新对智能体的心智模型"""
        if agent_id not in self.agent_models:
            raise ValueError(f"Unknown agent: {agent_id}")
    ...
    def _infer_belief_strength(self, agent_state: MentalState, proposition: str,
    ...
    def _check_expertise_relevance(self, agent_state: MentalState, 
    ...
    def _construct_perspective(self, observer_state: MentalState,
                               target_id: str) -> MentalState:
        """构建观察者视角下的目标心智模型"""
    ...
    def _get_visible_information(self, agent_state: MentalState,
    ...
    def _get_hidden_information(self, agent_state: MentalState,
                                 scenario: str) -> List[str]:
        """获取该智能体不知道的信息"""
        hidden = []
    ...
    def _infer_likely_beliefs(self, agent_state: MentalState,
                               scenario: str) -> List[str]:
        """推断可能的信念"""
        likely = []
    ...
    def _identify_knowledge_gaps(self, agent_state: MentalState,
                                  scenario: str) -> List[str]:
        """识别知识空白"""
        gaps = []
    ...
    def _update_beliefs_from_observation(self, agent_state: MentalState,
                                          observation: str) -> None:
        """从观察更新信念"""
        # 简化实现：将观察作为新信念添加
        agent_state.add_belief(observation, BeliefStrength.LIKELY, source='observation')
    
    def _update_intentions_from_action(self, agent_state: MentalState,
                                        action: str) -> None:
        """从行动更新意图"""
        # 基于行动推断并更新意图
        action_lower = action.lower()
...
class EmpathyModule:
    ...
    def resonate_emotion(self, observed_emotion: Emotion,
                         relationship: str = 'neutral') -> Emotion:
        """
        情感共鸣：感受他人的情绪
    ...
    def cognitive_empathy(self, target_mental_state: MentalState,
        ...
        understanding = {
            ...
            'key_beliefs': [b.content for b in target_mental_state.beliefs.values() 
                           if b.confidence > 0.7][:3],
    ...
    def _identify_stressors(self, mental_state: MentalState,
    ...
    def _assess_coping_needs(self, mental_state: MentalState) -> List[str]:
        ...
        # 检查信念不确定性
        uncertain_beliefs = [b for b in mental_state.beliefs.values() 
                            if b.confidence < 0.4]
...
class IntentionRecognizer:
    ...
    def recognize_from_speech(self, utterance: str, context: str = '') -> Dict[str, Any]:
        ...
        sorted_intentions = sorted(intention_scores.items(), 
    ...
    def recognize_from_behavior(self, actions: List[str], 
    ...
    def recognize_combined(self, utterance: Optional[str], actions: List[str],
                           context: str, speaker_profile: Dict[str, Any] = None) -> Dict[str, Any]:
        """综合言语、行为和语境识别意图"""
        speech_result = {}
        behavior_result = {}
        ...
        sorted_intentions = sorted(combined_scores.items(),
                                   key=lambda x: x[1], reverse=True)
...
class DeceptionDetector:
    ...
    def detect_deception(self, statement: str, context: str,
                         agent_id: Optional[str] = None,
                         nonverbal_cues: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        检测欺骗
...
class SocialLearningEngine:
    ...
    def observe_behavior(self, agent_id: str, behavior: str,
                         outcome: str, context: str) -> None:
        """观察并记录他人行为"""
        observation = {
            'agent_id': agent_id,
            'behavior': behavior,
    ...
    def learn_from_observation(self, learner_state: MentalState,
                               target_behavior: str) -> Optional[str]:
        """
        从观察中学习
        ...
        relevant_observations = [
            obs for obs in self.observed_behaviors
            if target_behavior in obs['behavior']
        ]
    ...
    def imitate(self, model_agent_id: str, behavior_pattern: List[str],
                learner_capabilities: Set[str]) -> List[str]:
        """
        模仿学习
    ...
    def learn_social_norm(self, context: str, behaviors: List[str],
                          sanctions: Dict[str, str]) -> None:
        """
        学习社会规范
...
class SocialNetworkModel:
    ...
    def establish_relationship(self, agent1: str, agent2: str,
    ...
    def update_trust(self, agent1: str, agent2: str, 
                     experience: str, delta: float) -> None:
        """根据经验更新信任度"""
    ...
    def propagate_belief(self, source_agent: str, belief: str,
                         initial_strength: float) -> Dict[str, float]:
        """
        信念在网络中的传播
...
class SocialCognitionEngine:
    ...
    def register_agent(self, agent_id: str, 
                       initial_beliefs: List[str] = None,
                       initial_desires: List[str] = None) -> MentalState:
        """注册智能体并返回其心智状态"""
    ...
    def understand_other(self, observer_id: str, target_id: str,
                         situation: str) -> Dict[str, Any]:
        """
        理解他人：综合心智理论和共情
        ...
        if target_state:
            emotional_understanding = self.empathy_module.cognitive_empathy(
    ...
    def interpret_action(self, agent_id: str, action: str,
                         context: str, utterance: str = None) -> Dict[str, Any]:
        """
        解释行动：识别意图并检测欺骗
    ...
    def learn_from_interaction(self, learner_id: str, model_id: str,
                               behavior: str, outcome: str,
    ...
    def _calculate_average_trust(self) -> float:
        ...
        total_trust = sum(
            rel['trust'] for rel in self.social_network.relationships.values()
        )
