...
class KnowledgeConsolidator:
    ...
    def add_to_short_term(self, knowledge: Dict):
        ...
        self.short_term_memory.append(
            {"content": knowledge, "timestamp": time.time(), "access_count": 1}
        )
    ...
    def consolidate(self) -> List[Dict]:
        ...
        sorted_memory = sorted(
            self.short_term_memory, key=lambda x: x["access_count"], reverse=True
        )
    ...
    def apply_mutation(self, mutation: Dict, validate_func: callable) -> bool:
        ...
        if not validate_func(mutation):
            self.rejected_mutations.append(
                {"mutation": mutation, "reason": "validation_failed", "timestamp": time.time()}
            )
            return False
        ...
        if success:
            self.applied_mutations.append(
                {"mutation": mutation, "applied_at": time.time(), "status": "success"}
            )
            return True
        else:
            self.rejected_mutations.append(
                {"mutation": mutation, "reason": "sandbox_test_failed", "timestamp": time.time()}
            )
            return False
...
class MetricCollector:
    ...
    def record(self, metric_name: str, value: float, context: Optional[Dict] = None):
        ...
        with self._lock:
            sample = MetricSample(
                timestamp=time.time(), metric_name=metric_name, value=value, context=context or {}
            )
            self.samples.append(sample)

    def get_metrics(
        self, metric_name: Optional[str] = None, time_range: Optional[Tuple[float, float]] = None
    ) -> List[MetricSample]:
        """获取指标"""
        with self._lock:
...
class ReportGenerator:
    ...
    def generate_daily_report(self, cycles: List[EvolutionCycle], metrics: MetricCollector) -> str:
        """生成日报"""
...
class LongTermEvolution:
    ...
    def start(self, simulated_days: int = 7):
        ...
        self._thread = threading.Thread(
            target=self._evolution_loop, args=(simulated_days,), daemon=True
        )
        self._thread.start()
    ...
    def _run_evolution_cycle(self) -> EvolutionCycle:
        ...
        curriculum = self.curriculum_generator.generate_curriculum(self.config["tasks_per_cycle"])
        ...
        # 记录指标
        self.metric_collector.record("task_success", cycle.success_rate, {"cycle": cycle.cycle_id})
        self.metric_collector.record(
            "knowledge_growth", cycle.knowledge_gained, {"cycle": cycle.cycle_id}
        )
        ...
        if cycle.metrics.get("improvement_needed"):
            mutation = {"type": "parameter_tuning", "target": "learning_rate", "change": "+0.01"}
    ...
    def _generate_reports(self):
        ...
        daily_report = self.report_generator.generate_daily_report(
            self.evolution_history, self.metric_collector
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_success_rate": (
                statistics.mean([c.success_rate for c in self.evolution_history])
...
def main():
    ...
    print(
        f"  Mutation Success Rate: {final_status['mutation_statistics']['success_rate']*100:.1f}%"
    )
