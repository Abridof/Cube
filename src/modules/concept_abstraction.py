...
class ConceptHierarchy:
    ...
    def __init__(self):
        self.nodes: Dict[str, Dict] = (
            {}
        )  # concept_id -> {name, level, parent, children, properties}
...
class MetaPatternRecognizer:
    ...
    def _abstract_relations(
        ...
        # 创建实体映射
        entity_map = {
            e: f"E{i}" for i, e in enumerate(set(e for rel in relations for e in [rel[0], rel[2]]))
        }
    ...
    def _update_concept_hierarchy(self, structure: AbstractStructure, case: ConcreteCase):
        ...
        for i, elem in enumerate(structure.elements):
            ...
            self.concept_hierarchy.add_concept(
                elem_key, elem, level=2, parent_id=structure_key, properties={"type": "element"}
            )
    ...
    def _graph_match(
        self, case1: ConcreteCase, case2: ConcreteCase
