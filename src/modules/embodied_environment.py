...
class ContinuousPhysicsEngine:
    ...
    def __init__(self, world_size: Tuple[float, float] = (100.0, 100.0),
                 gravity: float = 0.0, friction: float = 0.98):
...
class EmbodiedAgent:
    ...
    def __init__(self, agent_id: str, position: Vector2D, 
                 vision_range: float = 20.0, field_of_view: float = 360.0):
        self.id = agent_id
    ...
    def perceive(self, physics_engine: ContinuousPhysicsEngine) -> List[GameObject]:
        ...
        # 获取视野内的对象
        nearby = physics_engine.get_nearby_objects(
        ...
        for obj in nearby:
            ...
            if angle <= self.field_of_view / 2:
                # 检查视线是否被遮挡
                if physics_engine.check_line_of_sight(
                    self.game_object.position, 
                    obj.position
                ):
    ...
    def decide_action(self, perceived_objects: List[GameObject], 
                      dt: float = 1.0) -> Action:
        """基于内在动机决定行动"""
        ...
        if self.internal_state.energy < 40:
            food_objects = [o for o in perceived_objects 
                          if o.obj_type == ObjectType.FOOD]
            if food_objects:
                nearest_food = min(food_objects, 
                                  key=lambda x: x.position.distance_to(
                                      self.game_object.position))
        ...
        # 好奇驱动 (探索未知或预测误差大的区域)
        unknown_objects = [o for o in perceived_objects 
                          if o.id not in self.known_objects]
        if unknown_objects:
    ...
    def _generate_approach_action(self, targets: List[GameObject]) -> Action:
        ...
        return Action(
            ...
            parameters={"force_x": force.x, "force_y": force.y}
        )
        
    def _generate_exploration_action(self, 
                                     interesting_objects: List[GameObject] = None
                                    ) -> Action:
        """生成探索行动"""
...
class MultiAgentSociety:
    ...
    def remove_agent(self, agent_id: str):
        ...
        if agent_id in self.agents:
            self.physics_engine.remove_object(
                self.agents[agent_id].game_object.id
            )
            ...
            if msg.receiver_id is None:
                ...
                if msg.receiver_id in self.agents:
                    self._deliver_message(
                        self.agents[msg.receiver_id], msg
                    )
    ...
    def step(self, dt: float = 1.0):
        ...
        # 5. 更新预测模型
        for agent_id, agent in self.agents.items():
            agent.update_prediction_model({
                "position": agent.game_object.position
            })
...
class EmergenceAnalyzer:
    ...
    def calculate_mutual_information(self, series_a: List, 
                                     series_b: List) -> float:
        """计算互信息 (衡量两个序列的相关性)"""
    ...
    def analyze_emergence(self) -> Dict:
        ...
        if len(positions_over_time) > 1 and len(positions_over_time[0]) > 1:
            ...
            agent_1_x = [f[1][0] for f in positions_over_time 
                        if len(f) > 1]
            if agent_1_x:
...
def run_demo():
    ...
    for i in range(3):
        ...
        print(f"✓ 创建智能体 {agent.id} 在位置 ({agent.game_object.position.x:.1f}, {agent.game_object.position.y:.1f})")
    ...
    for step in range(100):
        ...
        if step % 20 == 0:
            ...
            print(f"Step {step:3d}: 平均能量={sum(energies)/len(energies):.1f}, "
