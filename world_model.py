...
class State:
    ...
    def get_value(self, var_name: str) -> Any:
        return self.variables.get(var_name, {}).value if isinstance(self.variables.get(var_name), StateVariable) else None
    ...
    def set_value(self, var_name: str, value: Any, var_type: StateType = StateType.ABSTRACT):
        if var_name in self.variables:
            ...
            self.add_variable(StateVariable(
                name=var_name,
                value=value,
                var_type=var_type
            ))
...
class WorldModel:
    ...
    def observe(self, variables: Dict[str, Tuple[Any, StateType]], 
                context: Optional[Dict] = None) -> State:
        """
        观测并创建新状态
        ...
        for var_name, (value, var_type) in variables.items():
            state.add_variable(StateVariable(
                name=var_name,
                value=value,
                var_type=var_type,
                source="sensor"
    ...
    def counterfactual(self, intervention: Dict[str, Any], 
                      compare_to: str = "actual") -> Counterfactual:
        """
        反事实推理："如果 X 发生，会怎样？"
        ...
        # 分析差异
        diff_analysis = self._analyze_difference(actual_outcome, 
                                                  cf_prediction.predicted_state if cf_prediction else cf_state)
    ...
    def update_self_model(self, capability: str = None, 
                         limitation: str = None,
    ...
    def load(self, filepath: str):
        ...
        self.causal_graph = defaultdict(set, {k: set(v) for k, v in data.get('causal_graph', {}).items()})
    ...
    def export_summary(self) -> Dict:
        ...
        return {
            ...
            'avg_prediction_accuracy': sum(self.prediction_accuracy) / len(self.prediction_accuracy) if self.prediction_accuracy else 0,
