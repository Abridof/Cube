...
from experiment_phase5 import (
    ...
    DataIngestionEngine,
    GridWorldAction, GridWorldCell, GridWorldState, GridWorldEnvironment,
    QLearningAgent,
    EmergenceObserver
)
...
class TestWikipediaSimulator(unittest.TestCase):
    ...
    def test_generate_article_history(self):
        ...
        has_history_terms = any(term in content_lower for term in 
                               ["occurred", "revolution", "war", "moment", "historians"])
    ...
    def test_generate_article_technology(self):
        ...
        # 技术类文章应该包含技术相关词汇
        content_lower = article.content.lower()
        has_tech_terms = any(term in content_lower for term in 
                           ["system", "components", "developed", "uses"])
    ...
    def test_generate_article_random(self):
        ...
        self.assertIsInstance(article, DataSample)
        self.assertIn(article.metadata["category"], 
...
class TestScientificPaperSimulator(unittest.TestCase):
    ...
    def test_paper_metadata(self):
        ...
        self.assertIn(paper.metadata["field"], 
                     ["computer vision", "natural language processing", 
                      "computational biology", "robotics", "materials science"])
        self.assertIn(paper.metadata["venue"], 
                     ["NeurIPS", "ICML", "CVPR", "ACL", "Nature"])
...
class TestDataIngestionEngine(unittest.TestCase):
    ...
    def test_run_ingestion_pipeline(self):
        """测试完整摄入流水线"""
        sources = {
            DataSourceType.WIKIPEDIA: 2,
            DataSourceType.CODE_REPO: 1
        }
...
class TestQLearningAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent = QLearningAgent(self.actions, learning_rate=0.1, 
