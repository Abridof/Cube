...
class ResourceManager:
    ...
    def allocate(self, cpu: int = 1, memory_mb: int = 512) -> bool:
        """分配资源"""
        with self._lock:
            if (self.allocated_cpu + cpu <= self.max_cpu and 
    ...
    def get_usage(self) -> Dict[str, float]:
        ...
        return {
            ...
            'memory_usage': self.allocated_memory_mb / self.max_memory_mb if self.max_memory_mb > 0 else 0
        }
...
class SystemOrchestrator:
    ...
    def _register_builtin_modules(self):
        ...
        # 定义核心模块列表
        builtin_modules = [
            ModuleInfo(name="ucr_layer", class_name="UnifiedRepresentationEngine", module_path="ucr_layer"),
            ModuleInfo(name="knowledge_graph", class_name="KnowledgeGraph", module_path="knowledge_graph"),
            ModuleInfo(name="multimodal_perception", class_name="MultimodalFusionEngine", module_path="multimodal_perception"),
            ModuleInfo(name="world_model", class_name="WorldModel", module_path="world_model"),
            ModuleInfo(name="neural_backend", class_name="NeuralSymbolicLearner", module_path="neural_backend"),
            ModuleInfo(name="cognitive_loop", class_name="CognitiveLoopController", module_path="cognitive_loop"),
            ModuleInfo(name="embodied_environment", class_name="MultiAgentSociety", module_path="embodied_environment"),
            ModuleInfo(name="self_reflection", class_name="SelfReflectionEngine", module_path="self_reflection"),
        ]
...
def main():
    ...
    print(f"  Resource Usage: CPU={status['resources']['cpu_usage']*100:.1f}%, Memory={status['resources']['memory_usage']*100:.1f}%")
